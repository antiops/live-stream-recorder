#!/bin/bash
# DLive.tv Stream Recorder

if [[ ! -n "$1" ]]; then
  echo "usage: $0 dlive_username [format] [loop|once] [interval]"
  exit 1
fi
...
while true; do
  ...
  while true; do
    ...
    echo "$LOG_PREFIX Try to get current live stream of dlive.tv/$1"

    # Get the m3u8 address with streamlink
    M3U8_URL=$(streamlink --stream-url "dlive.tv/$1" "$FORMAT")
  ...
  FNAME="dlive_${1}_$(date +"%Y%m%d_%H%M%S").ts"
